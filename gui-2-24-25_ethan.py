...
def setup_gui():
    global root, video_frame, label_text_var

    root = tk.Tk()
    root.title("Demo 1: myFUS")

    # Set window size (adjust width to fit panels)
    window_width = 1000  # Increased width to fit left panel
    window_height = 700  # Increased height to fit top panel
    root.geometry(f"{window_width}x{window_height}")

    # Create the **Top Panel** for messages
    top_panel = tk.Frame(root, height=50, bg="lightgray")  # Light gray background
    top_panel.pack(side="top", fill="x")  # Expands horizontally

    label_text_var = tk.StringVar()
    label_text_var.set("Messages will appear here")  # Placeholder text
    message_label = tk.Label(top_panel, textvariable=label_text_var, font=("Helvetica", 14), bg="lightgray")
    message_label.pack(pady=10)  # Add spacing inside the panel

    # Create the **Left Panel** for buttons
    left_panel = tk.Frame(root, width=200, bg="lightgray")  # Fixed width
    left_panel.pack(side="left", fill="y")  # Expands vertically

    # Create the **Main Video Frame** (does not change)
    video_frame = tk.Label(root)
    video_frame.pack(expand=True, fill="both")  # Expands to remaining space

    # Add buttons to the **Left Panel**
    buttons = [
        ("Mask Capture", lambda: change_function(1)),
        ("Save Mask", lambda: change_function(2)),
        ("Find Imaging Windows", lambda: change_function(3)),
        ("Save Imaging Window", lambda: change_function(4)),
        ("Display Mask", lambda: change_function(5)),
        ("Realign Probe", lambda: change_function(6)),
        ("Save Probe Coordinates", save_probe_coordinates),
    ]

    for text, command in buttons:
        button = tk.Button(left_panel, text=text, command=command, font=("Helvetica", 12))
        button.pack(pady=10, padx=10, fill="x")  # Make buttons fill the width

    return root

def change_function(index):
    """Placeholder function to change active function."""
    global label_text_var
    label_text_var.set(f"Active Function: {index}")

def save_probe_coordinates():
    """Placeholder function for saving probe coordinates."""
    global label_text_var
    label_text_var.set("Probe Coordinates Saved!")
...
def get_username():
    """Prompt the user for a username and create/use their folder."""
    root = tk.Tk()
    ...
    if not os.path.exists(user_folder):
        ...
        print(f"New user folder created: {user_folder}")
    else:
        print(f"Existing user folder found: {user_folder}")
...
# Global list to store probe coordinates
probe_coordinates = []
...
def function3(frame2):
    ...
    global probe_coordinates  # Store probe coordinates
...
def main():
    ...
    # Get the username before starting the GUI
    username, user_folder = get_username()
    ...
    def save_mask():
        ...
        # # Define file paths
        # mask_image_path = os.path.join(user_folder, "saved_mask.png")
        # mask_array_path = os.path.join(user_folder, "saved_mask.npy")
