...
class MyFUSApp:
    ...
    def setup_panels(self):
        self.top_panel = tk.Frame(self.root, height=240, bg="lightgray")
        self.top_panel.pack(side="top", fill="x")

        ### Thing that I added ###
        # Load the logo image
        original_logo_image = PhotoImage(
            file='decoders_logo.png')  # Replace 'path_to_your_logo.png' with the actual path to your image file
        ...
        self.logo_label = tk.Label(self.top_panel, image=self.logo_image, bg="lightgray")
        self.logo_label.pack(side="left", anchor="nw", pady=10, padx=10)  # Adjust padding as needed
        ###########################
        ...
        self.instruction_label = tk.Label(self.top_panel, textvariable=self.instruction_var, font=("Helvetica", 14),
                                          bg="lightgray", wraplength=1500)
        self.instruction_label.pack(pady=10)

        self.left_panel = tk.Frame(self.root, width=374, bg="lightgray")
        self.left_panel.pack(side="left", fill="y")

        self.mask_panel = tk.Frame(self.left_panel, width=240, height=200, bg="lightblue")
        self.mask_panel.pack(side="bottom", padx=20, pady=40)

        self.maskstats = tk.StringVar()
        self.maskstats.set(self.get_mask_data())
        tk.Label(self.mask_panel, textvariable=self.maskstats, font=("Helvetica", 14), bg="white").pack(padx=20,
                                                                                                        pady=20)
    ...
    def function2(self, frame):
        ...
        # continue with mask overlap calculations
        ...
        for contour in contours:
            cv2.polylines(modified_frame, [contour], isClosed=True, color=(203, 192, 255), thickness=2)
        ...
        if self.positions_collected < self.total_positions:
            ...
            try:
                ...
                for key, value in self.relative_positions.items():
                    positions_array = self.relative_positions[key]  # Access the array within the tuple and nested structure
                ...
                # make new folder for probecoords
