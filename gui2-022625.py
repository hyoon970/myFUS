...
class MyFUSApp:
    ...
    def setup_panels(self):
        ...
        self.instruction_label = tk.Label(self.top_panel, textvariable=self.instruction_var, font=("Helvetica", 14), bg="lightgray", wraplength=1500)
        ...
        tk.Label(self.mask_panel, textvariable=self.maskstats, font=("Helvetica", 14), bg="white").pack(padx=20, pady=20)
    ...
    def function3(self, frame):
        ...
        if self.positions_collected <= self.total_positions:
            ...
            corners, ids, rejected = aruco.detectMarkers(frame, self.aruco_dict, parameters=self.detector.getDetectorParameters())
            if ids is not None:
            ...
            self.update_instruction("All positions are saved, you can close the application or proceed to Realign Probe")
        return frame
    ...
    def function5(self, frame):
        ...
        for i in range(self.total_positions):
            if i in self.relative_positions and len(self.relative_positions[i]) > 0:
                cv2.polylines(frame, np.int32([self.relative_positions[i]]), True, (255, 0, 0), 2)
        corners, ids, rejected = aruco.detectMarkers(frame, self.aruco_dict, parameters=self.detector.getDetectorParameters())
        if corners:
            aruco.drawDetectedMarkers(frame, corners, ids)
        overlap_id = 0
            ...
            for i in range(self.total_positions):
                if i in self.relative_positions and len(self.relative_positions[i]) > 0:
                    ...
                    if np.all(diff <= tolerance):
                        ...
                        cv2.polylines(frame, np.int32([self.relative_positions[overlap_id]]), True, (0, 255, 0), 4)
                    else:
                        self.update_instruction("Align with blue highlighted square")
                        cv2.polylines(frame, np.int32([self.relative_positions[overlap_id]]), True, (255, 0, 0), 4)
        return frame
