...
class MyFUSApp:
    ...
    def update_buttons(self):
        ...
        if self.current_function_index == 0:
            ...
            if self.positions_collected < self.total_positions:
                ...
                self.buttons[4].config(state=tk.NORMAL)  # Enable "Realign Probe"
                # self.buttons[5].config(state=tk.NORMAL)  # Enable "Realign probe"
        else: #(if current_function_index == 5)
            self.buttons[4].config(state=tk.NORMAL)  # Enable "Realign Probe"
    ...
    def get_username(self):
        ...
        if not os.path.exists(self.userfolder):
            ...
            self.load_saved_mask()
            print(type(self.rgb_mask))
            self.current_function_index = 5
        self.startup_flag = 0

    def load_saved_mask(self):
        # Define the path to the saved mask
        mask_image_path = os.path.join(self.userfolder, "masks", f"saved_mask_{self.username}.png")

        # Check if the mask file exists
        if os.path.exists(mask_image_path):
            # Load the grayscale mask
            self.rgb_mask = cv2.imread(mask_image_path, cv2.IMREAD_GRAYSCALE)
            print(f"saved mask array type is {type(self.rgb_mask)}")
            if self.rgb_mask is None:
                print("Error loading saved mask. File may be corrupt.")
                return
            # Count non-zero values in the saved mask
            total_saved = np.count_nonzero(self.rgb_mask)
            print(f"Total non-zero pixels in the saved mask: {total_saved}")
            # # Convert to RGB if needed for `self.rgb_mask`
            self.rgb_mask = cv2.cvtColor(self.rgb_mask, cv2.COLOR_GRAY2BGR)
            # Optionally log or notify the user
            print(f"Mask loaded successfully from {mask_image_path}")
        else:
            print(f"Mask file not found at {mask_image_path}")
    ...
    def function2(self, frame):
        ...
        # total_saved = np.count_nonzero(saved_mask_indices)
    ...
    def function3(self, frame):
        ...
        if self.positions_collected <= self.total_positions:
            ...
            corners, ids, rejected = self.detector.detectMarkers(flipped_frame)
    ...
    # Probe re-align, pull saved probe coords and plot overlap
    def function5(self, frame):
        flipped_frame = cv2.flip(frame, 1)

        # reading saved coordinates

        input_file = os.path.join(self.userfolder, "probecoords", f"saved_probecoords_{self.username}.csv")
        if os.path.exists(input_file):
            ...
            try:
                ...
                for i in range(0, len(data), 4):
                    ...
                    self.relative_positions[i/4] = coords
                    print(f"coords {self.relative_positions[i/4]}")
                    # cv2.polylines(flipped_frame, [coords], isClosed=True, color=(0, 0, 255), thickness=2)
            ...
            for i in range(self.total_positions):
                if i in self.relative_positions and len(self.relative_positions[i]) > 0:
                    for corner in corners:
                        diff = np.abs(np.array(corner) - np.array(self.relative_positions[i]).reshape(4, 2))
        ...
        # if no markers detected, draw the probe position anyway
        else:
            for i in range(self.total_positions):
                if i in self.relative_positions and len(self.relative_positions[i]) > 0:
